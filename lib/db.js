...
async function updateItem(itemId, data) {
  return withClient(async (client) => {
    ...
    // Reabrir (voltar a 'available') uma peça já vendida e vinculada a um
    // pedido existente desfaz o histórico da venda sem cancelar o pedido —
    // bloqueado enquanto o vínculo existir.
    if (merged.status === 'available' && current.status === 'sold') {
      const { rows: linkedOrderRows } = await client.query(
        `SELECT 1 FROM order_items oi
         JOIN orders o ON o.id = oi.order_id
         WHERE oi.item_id = $1 AND o.status IN ('pending', 'confirmed', 'delivered')
         LIMIT 1`,
        [itemId]
      );
      if (linkedOrderRows.length > 0) {
        throw new Error('Esta peça não pode ser reaberta porque está vinculada a um pedido.');
      }
    }
...
async function deleteItem(itemId) {
  return withClient(async (client) => {
    try {
      ...
      // 23503 = violação de chave estrangeira — a peça está referenciada
      // em order_items (já apareceu em algum pedido) e não pode ser apagada.
      if (err.code === '23503') {
        throw new Error('Esta peça não pode ser excluída porque está vinculada a um pedido.');
      }
      throw err;
    }
  });
...
async function createOrderOnce(client, { customerName, phone, itemIds }) {
  ...
  try {
    const items = [];
    // Trava as peças sempre na mesma ordem (id ordenado), independente da
    // ordem em que a cliente montou o carrinho — dois pedidos concorrentes
    // com as mesmas peças então travam na mesma sequência, o que evita boa
    // parte dos deadlocks (não elimina, por isso o retry em createOrder).
